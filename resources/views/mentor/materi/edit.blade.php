...
<div class="page-heading">
    ...
    value="{{ old('tanggal_pertemuan', $materi->tanggal_pertemuan ? \Carbon\Carbon::parse($materi->tanggal_pertemuan)->format('Y-m-d') : '') }}">
                    ...
                    <div class="form-group mb-3">
                        ...
                        <input type="number" id="pertemuan_ke" name="pertemuan_ke" class="form-control @error('pertemuan_ke') is-invalid @enderror" value="{{ old('pertemuan_ke', $materi->pertemuan_ke) }}">
                        @error('pertemuan_ke')<div class="invalid-feedback">{{ $message }}</div>@enderror
                    </div>

                    <div class="form-group mb-3">
                        <label for="judul">Judul Materi</label>
                        <input type="text" id="judul" name="judul" class="form-control @error('judul') is-invalid @enderror" value="{{ old('judul', $materi->judul) }}">
                    ...
                    <div class="form-group mb-3">
                        ...
                        @if($materi->file_pdf)
                            <small class="text-muted">File sebelumnya: <a href="{{ asset('storage/' . $materi->file_pdf) }}" target="_blank">Lihat PDF</a></small>
                        @endif
                    </div>

                    <div class="form-group mb-3">
                        <label for="link_zoom">Link Zoom</label>
                        <input type="url" id="link_zoom" name="link_zoom" class="form-control @error('link_zoom') is-invalid @enderror" value="{{ old('link_zoom', $materi->link_zoom) }}">
                        @error('link_zoom')<div class="invalid-feedback">{{ $message }}</div>@enderror
                    </div>

                    <div class="form-group mb-3">
                        <label for="link_rekaman">Link Rekaman</label>
                        <input type="url" id="link_rekaman" name="link_rekaman" class="form-control @error('link_rekaman') is-invalid @enderror" value="{{ old('link_rekaman', $materi->link_rekaman) }}">
